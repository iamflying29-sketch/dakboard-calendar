...
var NO_FEATURES = { anvil: 0, velum: 0 };
...
function transferDecode(c) {
  return c <= 0.04045 ? c / 12.92 : Math.pow((c + 0.055) / 1.055, 2.4);
}
function transferEncode(c) {
  return c <= 31308e-7 ? c * 12.92 : 1.055 * Math.pow(c, 1 / 2.4) - 0.055;
}
function convert(m, c) {
...
function srgbToDisplayP3(c) {
  return convert(SRGB_TO_DISPLAY_P3, c);
}
function displayP3ToSrgb(c) {
...
function isCelestialId(v) {
  return typeof v === "string" && CELESTIAL_IDS.includes(v);
}
...
function isFilterId(v) {
  return typeof v === "string" && FILTER_IDS.includes(v);
}
...
function resolveFilter(input) {
  ...
  const base = input.id ? FILTER_PRESETS[input.id] ?? NO_FILTER : { amount: 1, tint: [1, 1, 1], saturation: 0, lift: 0 };
  return toFilter({
...
function isPolarizerId(v) {
  return typeof v === "string" && POLARIZER_IDS.includes(v);
}
...
function resolvePolarizer(input) {
  ...
  return toPolarizer({
    strength: clamp012(input.strength ?? base.strength),
    ...
    saturation: clamp012(input.saturation ?? base.saturation),
    stopLoss: clamp012(input.stopLoss ?? base.stopLoss)
  });
}
...
function isToneId(v) {
  return typeof v === "string" && TONE_IDS.includes(v);
}
...
function isWeatherId(v) {
  return typeof v === "string" && WEATHER_IDS.includes(v);
}
...
function resolveWeather(input) {
  const w = typeof input === "string" ? WEATHER_PRESETS[input] ?? WEATHER_PRESETS.clear : input ?? {};
  ...
  const convection = clamp013(w.convection ?? 0);
  let clouds;
  let cloudCover;
  if (w.clouds != null) {
    ...
    cloudCover = clamp013(w.cloudCover ?? 0);
  ...
  return {
    clouds,
    features: {
      anvil: clamp013(w.features?.anvil ?? auto.anvil),
      velum: clamp013(w.features?.velum ?? auto.velum)
    },
    cloudCover,
    rain,
    snow,
    wind: windCurve(w.windSpeed ?? 0),
    thunder: clamp013(w.thunder ?? 0),
    haze
  };
}
...
var Atmosphere = class {
  ...
  startLoop() {
    ...
    const tick = (t) => {
      ...
      const interval = 1e3 / (this.opts.fps ?? 30);
...
// src/cubemap.ts
function renderCubeFaces(conditions = {}, options = {}) {
...
function weatherLabel(id, locale = "en") {
  return WEATHER_LABELS[locale][id];
}
function filterLabel(id, locale = "en") {
  return FILTER_LABELS[locale][id];
}
function cloudGenusLabel(id, locale = "en") {
